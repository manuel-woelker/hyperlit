...
fn get_string_or(table: &toml_span::value::Table, key: &str, default: &str) -> HyperlitResult<String> {
    match table.get(key) {
        None => {
...
fn get_string_array(table: &toml_span::value::Table, key: &str) -> HyperlitResult<Vec<String>> {
    match table.get(key) {
        ...
        Some(value) => {
            value.as_array()
                .ok_or_else(|| err!("{} is not an array", key))?.iter()
                .map(|v| v.as_str().ok_or_else(|| err!("{} is not a string", key)).map(|s| s.to_string()))
                .collect()
        }
    }
}
...
mod tests {
    ...
    fn read_config_with_defaults() -> HyperlitResult<()> {
        let config = HyperlitConfig::from_str(r#"
